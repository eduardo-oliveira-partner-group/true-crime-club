...
export function ProductDetailPageClient({
  ...
  const loadProduct = useCallback(
    async (showLoading = true) => {
      ...
      try {
        const [nextProduct, productsList] = await Promise.all([
        ...
        setRelated(
          nextProduct
            ? productsList.filter(
                (item) =>
                  nextProduct.relatedProductIds?.includes(item.id) &&
    ...
    return (
      <DesignPageShell showOverlays={false}>
        <section className="bg-(--paper)">
          <LoadErrorState
            code="Ficha temporariamente fechada"
            title="Não foi possível abrir o produto"
            description="A conexão com o arquivo foi interrompida. Tente novamente para reabrir esta ficha."
            onRetry={() => loadProduct(false)}
            className="min-h-[min(760px,calc(100svh-110px))]"
            secondaryLink={{ href: '/loja', label: 'Voltar à loja' }}
          />
        </section>
      </DesignPageShell>
    )
