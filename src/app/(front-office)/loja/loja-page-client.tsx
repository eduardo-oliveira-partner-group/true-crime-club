...
function pickFeaturedBox(boxProducts: Product[]): Product | undefined {
  return (
    boxProducts.find((product) => product.slug === highlightedEditionSlug) ??
...
function FeaturedProductSkeleton() {
  return (
    <div
      className="rounded-[14px_14px_16px_16px] border border-[rgba(33,28,24,0.15)] bg-(--card) p-4 sm:p-5"
      aria-busy="true"
      aria-label="Carregando produto em destaque"
    >
      <Skeleton className="aspect-4/3 w-full rounded-[4px_4px_10px_10px] bg-(--ink)/10" />
      <div className="grid gap-5 pt-5 sm:grid-cols-[1fr_auto] sm:items-end">
        <div>
          <Skeleton className="h-3 w-36 bg-(--ink)/10" />
          <Skeleton className="mt-3 h-7 w-56 max-w-full bg-(--ink)/10" />
          <Skeleton className="mt-2 h-4 w-full max-w-sm bg-(--ink)/10" />
        </div>
        <Skeleton className="h-11 w-32 bg-(--ink)/10" />
      </div>
    </div>
  )
}
...
function CatalogSkeleton() {
  return (
    <section
      className={`${sectionFrame} py-14 lg:py-20`}
      aria-busy="true"
    ...
    >
      ...
      <div className="mt-10 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
        {Array.from({ length: 6 }).map((_, index) => (
          <div
            key={index}
            className="rounded-[14px] border border-[rgba(33,28,24,0.15)] bg-(--card) p-4"
          >
            <Skeleton className="aspect-4/3 w-full rounded-[8px] bg-(--ink)/10" />
            <Skeleton className="mt-4 h-5 w-3/4 bg-(--ink)/10" />
            <Skeleton className="mt-2 h-4 w-1/2 bg-(--ink)/10" />
          </div>
        ))}
      </div>
    </section>
