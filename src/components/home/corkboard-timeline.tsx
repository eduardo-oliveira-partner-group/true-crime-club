...
import { IconCreditCard, IconPackage, IconChecklist } from '@tabler/icons-react'
  ...
  return (
    <div className="relative mx-auto max-w-5xl">
      ...
      */}
      <svg 
        width="100%"
        className="absolute top-0 left-0 right-0 h-1 hidden md:block z-10 pointer-events-none opacity-45 overflow-visible"
        aria-hidden="true"
      >
        <motion.line
          animate={{
      ...
      {steps.map((step, index) => {
        ...
        return (
          <motion.div
            key={`pin-${index}`}
            className="absolute size-3.5 rounded-full z-20 pointer-events-none hidden md:block"
            animate={{
              left: x,
              top: y,
            }}
            ...
            style={{
              ...
              boxShadow: '0 3px 5px rgba(33, 28, 24, 0.45), inset 0 -2px 3px rgba(0, 0, 0, 0.3)',
            }}
            aria-hidden="true"
          />
        )
            ...
            <ScrollRevealItem key={step.title} className="h-full">
              <motion.article
                animate={{
                  ...
                  rotate: hoveredIndex === index ? 0 : CARD_ROTATIONS[index] ?? 0,
                }}
                transition={SPRING_TRANSITION}
                className={cn(
              ...
              >
                ...
                <div 
                  className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 size-3.5 rounded-full z-30 pointer-events-none md:hidden"
                  style={{
                    background: `radial-gradient(circle at 35% 30%, rgba(255, 255, 255, 0.7) 0%, ${stepColor} 55%, rgba(0, 0, 0, 0.4) 100%)`,
                    boxShadow: '0 3px 5px rgba(33, 28, 24, 0.45), inset 0 -2px 3px rgba(0, 0, 0, 0.3)',
                  }}
                ...
                <div className="flex justify-between items-center text-[10px] font-mono tracking-wider text-[#d7c9b5]/40 border-b border-[#fffaf0]/8 pb-3 mb-5">
                ...
                <div 
                  className={cn(
                    "absolute right-4 bottom-14 font-heading text-4xl font-black select-none pointer-events-none tracking-widest opacity-[0.03] transition-opacity duration-300 group-hover:opacity-[0.06] border-2 px-3 py-1 rotate-[-12deg]",
                    index === 0 ? "text-[#1aa587] border-[#1aa587]" : index === 1 ? "text-[#5e5ea2] border-[#5e5ea2]" : "text-[#d84132] border-[#d84132]"
                  )}
                >
                  {meta.stamp}
                </div>
                ...
                <div className="flex-1 flex flex-col relative z-20">
                  {/* Pinned Card Icon */}
                  <div className="mb-4 inline-flex size-11 items-center justify-center bg-[#211c18] border border-[#fffaf0]/8 text-[#d7b56d] transition-all duration-300 group-hover:bg-[#d84132]/10 group-hover:border-[#d84132]/30 group-hover:text-[#ffb0a5]">
                    <StepIcon className="size-5 transition-transform duration-300 group-hover:scale-110 group-hover:rotate-6" />
                  </div>

                  <h3 className="font-heading text-lg font-bold tracking-tight text-[#fffaf0] group-hover:text-[#d84132] transition-colors duration-300">
                    {index + 1}. {step.title}
                  </h3>
                ...
                <div className="mt-6 pt-3 border-t border-[#fffaf0]/8 flex justify-between items-center text-[9px] font-mono text-[#d7c9b5]/20">
                  <span>CONFIDENTIALITY ASSURED</span>
                  <span className="font-semibold text-[#d84132]/40">PASSO 0{index + 1}</span>
                </div>
              </motion.article>
            </ScrollRevealItem>
          )
